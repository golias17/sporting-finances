...
test.describe("Compare Tab", () => {
  ...
  test("should toggle between Two seasons and Vs average modes", async ({
    page,
  }) => {
  ...
  test("should show average window options when in average mode", async ({
    page,
  }) => {
    ...
    await expect(page.locator(".cmp-average-excluded")).toContainText(
      "excludes",
    );
  });

  test("should update header when switching average windows", async ({
    page,
  }) => {
    // Switch to average mode
    const toggleBtn = page.locator(".cmp-controls .btn-preset").first();
  ...
  test("should display correct column labels", async ({ page }) => {
    ...
    await expect(page.locator(".cmp-col-header-label").first()).toContainText(
      "Season A",
    );

    // Check Season B label
    await expect(page.locator(".cmp-col-header-label").last()).toContainText(
      "Season B",
    );
  });
  ...
  test("should show average labels in column headers when in average mode", async ({
    page,
  }) => {
    // Switch to average mode
    const toggleBtn = page.locator(".cmp-controls .btn-preset").first();
