...
describe("localization.js", () => {
  it("should translate direct map notes exactly", () => {
    const pt = translateNote("Record sales season at the time. Marcos Rojo sold to Manchester United for €20M just one year after signing. Eric Dier and Cedric Soares also departed for the Premier League.");
    expect(pt).toBe("Época recorde de vendas à data. Venda de Marcos Rojo ao Manchester United por €20M um ano após a contratação. Eric Dier e Cédric Soares também saíram para a Premier League.");
  });
  ...
  it("should apply sequential replacements for recurring phrases", () => {
    ...
    expect(pt).toBe("Contratação de inverno. Contrato rescindido por mútuo acordo.");
  });
